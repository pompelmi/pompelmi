...
export async function checkFile(input, filename) {
    let name;
    let buf;
    if (typeof Buffer !== 'undefined' && Buffer.isBuffer(input)) {
        if (!filename) {
            throw new Error('Quando passi un Buffer devi fornire anche il nome del file come secondo argomento.');
        }
        name = filename;
        buf = input;
    }
    ...
    const result = (await scanAsync({ buffer: buf }));
    if (result.rules && result.rules.length > 0) {
        const ids = result.rules.map((r) => r.id).join(', ');
