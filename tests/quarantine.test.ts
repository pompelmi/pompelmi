...
import { FilesystemQuarantineStorage, QuarantineManager } from "../src/quarantine/index";
...
describe("QuarantineManager (FilesystemQuarantineStorage)", () => {
  ...
  it("shouldQuarantine returns true for suspicious and malicious", () => {
    ...
    expect(
      manager.shouldQuarantine({
        ...makeSuspiciousReport(),
        verdict: "clean",
        ok: true,
        matches: [],
      }),
  ...
  it("listPending returns only pending entries", async () => {
    await manager.quarantine(sampleBytes, makeSuspiciousReport(), {
      originalName: "a.pdf",
      sizeBytes: 4,
    });
    await manager.quarantine(sampleBytes, makeMaliciousReport(), {
      originalName: "b.pdf",
      sizeBytes: 4,
    });
  ...
  it("resolve with delete transitions to deleted and removes bytes", async () => {
    const created = await manager.quarantine(sampleBytes, makeMaliciousReport(), {
  ...
  it("resolve throws if already resolved", async () => {
    ...
    await expect(manager.resolve(created.id, { decision: "delete" })).rejects.toThrow(
      "already resolved",
    );
  });

  it("report returns correct counts", async () => {
    const e1 = await manager.quarantine(sampleBytes, makeSuspiciousReport(), {
      originalName: "a.pdf",
      sizeBytes: 4,
    });
    await manager.quarantine(sampleBytes, makeMaliciousReport(), {
