...
export class DynamicTaintEngine implements TaintCapableEngine {
  ...
  async isTainted(address: string): Promise<boolean> {
    ...
    return (
      this.memoryState.has(sanitizedAddress) ||
   ...
   */
  private async identifyEntryPoints(binaryPath: string): Promise<
    Array<{
      address: string;
      type: TaintSource;
      description: string;
   ...
   */
  private async initializeTaintSources(
    entryPoints: Array<{
      address: string;
      type: TaintSource;
      description: string;
  ...
  private analyzeVulnerabilities(flows: TaintFlow[]): TaintFlow[] {
    return flows.map((flow) => {
      ...
      const hasSanitization = flow.path.some(
        (step) => step.operation === "sanitization" || step.operation === "validation",
      );
  ...
  private getTotalSources(): number {
    return Array.from(this.taintLabels.values()).filter((label) =>
      this.config.enabledSources.includes(label.source),
    ).length;
  }
