...
export class TaintPolicyManager {
  ...
  getPoliciesByUseCase(useCase: TaintPolicy["useCase"]): TaintPolicy[] {
    return Array.from(this.policies.values()).filter((policy) => policy.useCase === useCase);
  }
  ...
  private initializePredefinedPolicies(): void {
    ...
    this.policies.set("vulnerability-assessment", {
      ...
      taintConfig: {
        ...
        enabledSources: ["user_input", "file_read", "network_recv", "environment"],
        enabledSinks: ["exec_function", "sql_query", "format_string", "memory_alloc", "file_write"],
    ...
    this.policies.set("fast-screening", {
      ...
      taintConfig: {
        ...
        enabledSources: ["user_input", "network_recv"],
        enabledSinks: ["exec_function", "network_send"],
