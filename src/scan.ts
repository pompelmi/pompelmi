...
function computeVerdict(matches: YaraMatch[]): Verdict {
  ...
  const anyHigh = matches.some(
    (m) => (m.tags ?? []).includes("critical") || (m.tags ?? []).includes("high"),
  );
...
export async function scanBytes(input: Uint8Array, opts: ScanOptions = {}): Promise<ScanReport> {
  ...
  const matchesH = await (typeof (scanFn as any) === "function"
    ? (scanFn as any)
    : (scanFn as any).scan)(input, ctx);
  ...
  const advancedEnabled =
    opts.enableAdvancedDetection ?? opts.config?.advanced?.enablePolyglotDetection ?? true;
  if (advancedEnabled) {
