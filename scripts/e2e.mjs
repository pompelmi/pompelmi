...
function runCli(cli, filePath) {
  try {
    const cmd =
      cli === "pompelmi"
        ? `pompelmi "${filePath}" --format table`
        : `node "${cli}" "${filePath}" --format table`;
    const out = sh(cmd);
...
(async function main() {
  ...
  if (!dist) {
    console.error("❌ dist/* not found. Build should emit dist/*");
    process.exit(2);
  }
  console.log("→ bundle:", dist);

  try {
    const mod = await importDist(dist);
    const keys = Object.keys(mod || {});
    console.log("→ exports:", keys.join(", ") || "(none)");
  } catch (e) {
    console.log("! import dist failed (continuing to CLI):", e?.message || e);
  }

  const cli = findCli();
  if (!cli) {
    console.log("→ CLI not found. Skipping CLI checks.\n✅ E2E completed (bundle OK).");
    return;
  }
  console.log("→ CLI:", cli);
