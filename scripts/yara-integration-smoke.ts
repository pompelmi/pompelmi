...
async function collect(caseName: string, opts: CaseOpts) {
  ...
  for await (const entry of scanDir(dir, opts as any)) {
    const name =
      (entry.path && basename(entry.path)) || (entry.absPath && basename(entry.absPath)) || "";
...
function get(results: Record<string, CaseRow>, file: string): CaseRow & { matches: string[] } {
...
expect(
  hasRule(get(case1, "tail_virus.txt")),
  "case1: tail_virus.txt should match (full file path)",
);
...
// Assertions
{
  ...
  expect(
    bin.status === "skipped" && bin.reason === "filtered-ext",
    "case3: binary.bin should be skipped with reason=filtered-ext",
  );
...
["big.txt", "head_virus.txt", "tail_virus.txt"].forEach((f) => {
  ...
  expect(
    row.status === "skipped" && row.reason === "max-size",
    `case4: ${f} should be skipped with reason=max-size`,
  );
});
...
// Assertions
{
  ...
  expect(
    tail.status === "scanned" && tail.mode === "buffer-sampled",
    "case5: tail_virus.txt should be scanned via buffer-sampled",
  );
  expect(!hasRule(tail), "case5: tail_virus.txt should NOT match with 16-byte sampling");

  const head = get(case5, "head_virus.txt");
  expect(
    head.status === "scanned" && head.mode === "buffer-sampled",
    "case5: head_virus.txt should be scanned via buffer-sampled",
  );
  expect(hasRule(head), "case5: head_virus.txt SHOULD match (virus at head of file)");

  const big = get(case5, "big.txt");
  expect(
    big.status === "scanned" && big.mode === "buffer-sampled",
    "case5: big.txt should be scanned via buffer-sampled",
  );
...
// Assertions
{
  ...
  expect(
    tail.status === "scanned" && tail.mode === "buffer-sampled",
    "case6: tail_virus.txt should be scanned via buffer-sampled",
  );
  ...
  expect(
    big.status === "scanned" && big.mode === "buffer-sampled",
    "case6: big.txt should be scanned via buffer-sampled",
  );
...
// Assertions
{
  ...
  expect(
    ["async", "file"].includes(big.mode ?? ""),
    "case6b: big.txt mode should be async or file",
  );
}
...
// Assertions
{
  ...
  expect(
    ["file", "buffer", "buffer-sampled"].includes(big.mode ?? ""),
    "case7: big.txt mode should be file/buffer/buffer-sampled",
  );
}
