 ...
 */
export function formatTable(results: ScanResult[], summary: ScanSummary, baseDir: string): string {
  const output: string[] = [];
  ...
  if (threats.length > 0) {
    ...
    threats.forEach((result) => {
      const verdict =
        result.verdict === "malicious" ? pc.red(pc.bold("MALICIOUS")) : pc.yellow("SUSPICIOUS");

      const findings =
        result.findings.length > 0
          ? result.findings.slice(0, 2).join(", ") +
            (result.findings.length > 2 ? ` (+${result.findings.length - 2})` : "")
          : "—";
  ...
  if (errors.length > 0) {
    ...
    errors.forEach((result) => {
      table.push([relative(baseDir, result.file), pc.red(result.error || "Unknown error")]);
    });
 ...
 */
export function formatJson(results: ScanResult[], summary: ScanSummary, baseDir: string): string {
