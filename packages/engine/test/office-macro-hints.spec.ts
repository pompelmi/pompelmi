...
describe("OfficeMacroHintsScanner", () => {
  ...
  it("clean for OOXML without macro hints", async () => {
    const res = await run([
      lfh("[Content_Types].xml"),
      cen("[Content_Types].xml"),
      lfh("xl/workbook.xml"),
      cen("xl/workbook.xml"),
      lfh("xl/worksheets/sheet1.xml"),
      cen("xl/worksheets/sheet1.xml"),
    ]);
    expect(res.verdict).toBe("clean");
  });
  ...
  it("suspicious when vbaProject.bin present", async () => {
    const res = await run([
      ...
      lfh("xl/workbook.xml"),
      cen("xl/workbook.xml"),
      lfh("xl/vbaProject.bin"),
      cen("xl/vbaProject.bin"),
    ]);
  ...
  it("suspicious when macrosheets present", async () => {
    const res = await run([
      lfh("[Content_Types].xml"),
      cen("[Content_Types].xml"),
      lfh("xl/macrosheets/sheet1.xml"),
      cen("xl/macrosheets/sheet1.xml"),
    ]);
    expect(res.verdict).toBe("suspicious");
  });
});
