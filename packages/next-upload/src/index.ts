...
export type ScanEvent =
  ...
  | {
      type: "archive_limit";
      archive?: string;
      reason: "max_entries" | "max_total" | "max_entry" | "nested_zip";
    }
...
async function resolveCreateYaraScannerFromRules(): Promise<
  (rules: string) => Promise<PompelmiScanner>
> {
  try {
  ...
  throw new Error(
    "pompelmi:createYaraScannerFromRules non trovato. Aggiorna 'pompelmi' o passa 'scanner'.",
  );
}
...
async function scanZipBuffer(
  ...
  opts: {
    ...
    scanHidden: boolean;
    depth?: number;
    mapMatchesToVerdict: (m: YaraMatch[]) => Verdict;
  },
): Promise<Verdict> {
  const {
    scanner,
      ...
      for (const f of files) {
        ...
        if (includeExtensions) {
          ...
          if (!e || !includeExtensions.includes(e)) {
            const payload = {
              ok: false,
              reason: "extension_not_allowed" as const,
              filename,
              allowed: includeExtensions,
            };
