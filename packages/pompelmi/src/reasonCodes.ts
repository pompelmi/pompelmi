...
export function inferReasonCode(finding: string): ReasonCode {
  ...
  if (lower.includes("embedded") && lower.includes("script"))
    return ReasonCode.FILE_EMBEDDED_SCRIPT;
  ...
  if (lower.includes("zip bomb") || lower.includes("archive bomb"))
    return ReasonCode.ARCHIVE_BOMB_DETECTED;
  if (lower.includes("path traversal") || lower.includes("traversal"))
    return ReasonCode.ARCHIVE_PATH_TRAVERSAL;
