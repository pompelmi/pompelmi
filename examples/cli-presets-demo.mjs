...
import { getPreset, getReasonCodeInfo, listPresets, ReasonCode, scan } from "pompelmi";
...
function printPresetInfo(presetName) {
  ...
  console.log(
    colorize(
      `║ ${colorize(`Preset: ${presetName.toUpperCase()}`, colors.bold).padEnd(width - 2)}║`,
      colors.gray,
    ),
  );
  console.log(colorize(`╠${"═".repeat(width)}╣`, colors.gray));
  console.log(
    colorize(`║ Max Depth:           ${String(preset.maxDepth).padEnd(width - 23)}║`, colors.gray),
  );
  console.log(
    colorize(
      `║ Max Buffer:          ${(preset.maxBufferSize / 1024 / 1024).toFixed(0)}MB`.padEnd(width) +
        "║",
      colors.gray,
    ),
  );
  console.log(
    colorize(
      `║ Heuristic Threshold: ${preset.heuristicThreshold}/100`.padEnd(width) + "║",
      colors.gray,
    ),
  );
  console.log(
    colorize(
      `║ Fail Fast:           ${preset.failFast ? "Yes" : "No"}`.padEnd(width) + "  ║",
      colors.gray,
    ),
  );
  console.log(colorize(`╚${"═".repeat(width)}╝`, colors.gray));
...
function formatScanResult(result, fileName) {
  ...
  console.log(
    `${colorize("Verdict:", colors.bold)} ${colorize(result.verdict.toUpperCase(), verdictColors[result.verdict])}`,
  );
  ...
  if (result.findingsWithReasons && result.findingsWithReasons.length > 0) {
    ...
    for (const finding of result.findingsWithReasons) {
      ...
      if (finding.metadata) {
        console.log(
          `    ${colorize("metadata:", colors.gray)} ${JSON.stringify(finding.metadata)}`,
        );
      }
    }
  } else {
...
function determineAction(result) {
  ...
  const hasMalware = reasonCodes.some(
    (code) => code.startsWith("MALWARE_") && code !== "MALWARE_EICAR_TEST",
  );
...
function printAction(action) {
  ...
  console.log(
    `${colorize("Decision:", colors.bold)} ${colorize(action.action.toUpperCase(), actionColors[action.action])}`,
  );
...
async function showPresets() {
  ...
  for (const presetName of presets) {
    ...
    console.log(
      `  Max depth: ${preset.maxDepth} | Buffer: ${(preset.maxBufferSize / 1024 / 1024).toFixed(0)}MB | Threshold: ${preset.heuristicThreshold}/100 | Fail fast: ${preset.failFast ? "Yes" : "No"}`,
    );
    console.log();
  }
}
