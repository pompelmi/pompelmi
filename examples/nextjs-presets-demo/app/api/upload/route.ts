...
import { getReasonCodeInfo, ReasonCode, scan } from "pompelmi";
...
export async function POST(req: NextRequest) {
  try {
    ...
    const reasonCodes =
      result.findingsWithReasons?.map((f) => ({
        code: f.reasonCode,
        message: f.message,
        metadata: f.metadata,
    ...
    const hasMalware = reasonCodes.some(
      (r) => r.code.startsWith("MALWARE_") && r.code !== "MALWARE_EICAR_TEST",
    );

    const hasEicar = reasonCodes.some((r) => r.code === "MALWARE_EICAR_TEST");
