...
function resolveScanner(mod: any) {
  ...
  if (
    typeof mod.composeScanners === "function" &&
    typeof mod.createHeuristicsScanner === "function"
  ) {
    const built = mod.createHeuristicsScanner();
    const h = wrapCallable(built);
    if (h) {
      const composed = mod.composeScanners([["heuristics", h]], {
        parallel: false,
        stopOn: "suspicious",
      });
  ...
  throw new Error(
    `@pompelmi/engine-heuristics: no callable scanner export found (exports: ${Object.keys(mod || {}).join(", ")})`,
  );
}
  ...
  try {
    ...
    if (!files.length) {
      return NextResponse.json(
        { error: "no_files", message: "No files in multipart request" },
        { status: 400 },
      );
    }
    ...
    for (const f of files) {
      ...
      if (typeof f.size === "number" && f.size > policy.maxFileSizeBytes) {
        return NextResponse.json(
          { error: "file_too_large", message: `File "${originalname}" exceeds max allowed size` },
          { status: 422 },
        );
      }
          ...
          return NextResponse.json(
            {
              ...
              message: `File "${originalname}" has disallowed extension ".${e}"`,
            },
            { status: 422 },
          );
        }
        ...
        return NextResponse.json(
          {
            ...
            message: `File "${originalname}" has disallowed MIME "${mime}"`,
          },
          { status: 422 },
        );
      }
    ...
    if (shouldBlock) {
      return NextResponse.json(
        { error: "blocked_by_policy", message: `Upload blocked (${overall}).`, results },
        { status: 422 },
      );
    }
    ...
    if (policy.failClosed) {
      return NextResponse.json(
        { error: "scan_error", message: err?.message || "Upload rejected by scanner" },
        { status: 422 },
      );
    }
    throw err;
